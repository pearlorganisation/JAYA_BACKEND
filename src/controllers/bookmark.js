...
export const getBookmarksByUser = asyncHandler(async (req, res) => {
  ...
  const bookmark = await bookMark.find({ userId }).populate('bookmarks','title miniTitle tags').lean();
...
export const addSchemeToBookmarks = asyncHandler(async (req, res) => {
  const { userId, schemeId } = req.body;
   
  console.log("fsdfsd",req.body);
  if (!userId || !schemeId) {
    res.status(400);
